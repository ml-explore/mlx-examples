...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert M2M style model weights to MLX."
    )
    parser.add_argument(
        "--nllb-model",
        type=str,
    ...
    parser.add_argument("--input_sentence", type=str, default="Hello there!")
    args = parser.parse_args()

    current_directory = os.getcwd()
    weights_path = os.path.join(
        current_directory, "weights", args.model_name.replace("/", "-") + ".npz"
    )
