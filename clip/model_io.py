...
def map_clip_text_weights(key, value):
    # Remove prefixes
...
def map_clip_vision_weights(key, value):
    # Remove prefixes
...
def load_text_encoder(key: str = _DEFAULT_MODEL):
    ...
    _load_safetensor_weights(map_clip_text_weights, model, model_weights_path)

    return model


def load_vision_encoder(key: str = _DEFAULT_MODEL):
    # Download the config and create the model
    config = _MODELS[key]["config"]
    with open(hf_hub_download(key, config)) as f:
        config = json.load(f)
