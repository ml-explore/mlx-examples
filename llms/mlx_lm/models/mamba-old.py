...
class ModelArgs(BaseModelArgs):
    ...
    dt_rank: Union[int, str] # time_step_rank
    d_model: int
    d_inner: int
    vocab_size: int
    n_layer: int
...
class MambaBlock(nn.Module):
    ...
    def ssm_step(self, x, h):
        A = -mx.exp(self.A_log)
        D = self.D
        deltaBC = self.x_proj(self.norm(x))
    ...
    def ssm_new(self, x):
        ...
        delta = nn.softplus(self.dt_proj(delta))
        return self.selective_scan_new(x, delta, A, B, C, D)
    ...
    def selective_scan_new(self, u, delta, A, B, C, D):
        (b, l, d_in) = u.shape
        n = A.shape[1]
    ...
    def new(self, x):
        ...
        out = self.ssm_new(nn.silu(x)) * nn.silu(r)
