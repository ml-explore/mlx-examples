...
class NanoLlavaModel(nn.Module):
    ...
    def get_input_embeddings(
        ...
        *_, image_forward_outs = self.vision_tower(
            pixel_values.transpose(0, 2, 3, 1), output_hidden_states=True
        )

        image_features = image_forward_outs.hidden_state[-1].dtype(pixel_values.dtype)
        assert image_features.shape[-2] == 729

        image_features = self.mm_projector(image_features)

        final_inputs_embeds = self._prepare_inputs_for_multimodal(
            image_features, input_embeds, input_ids
        )
    ...
    def _prepare_inputs_for_multimodal(
        ...
        for position in image_positions:
            text_segments.append(input_embeds[:, starts_idx:position])
            starts_idx = position + 1

        image_embeddings = mx.split(image_features, image_features.shape[0])
        final_embeddings = [v for p in zip(text_segments, image_embeddings) for v in p]
        final_embeddings += [input_embeds[:, starts_idx:]]
    ...
    def __call__(
        ...
        logits, cache = self.language_model(
            input_ids, cache=cache, input_embeds=input_embeddings
        )
        return logits, cache
